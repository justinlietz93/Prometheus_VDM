...
## Dataset
- H5 path: /home/justin/git/Prometheus_VDM/vdm_rt_v9/runs/20260413_142439_N1000_pulse/run_log.h5
- Nodes: 1000
...
## Executive Findings
- The run shows a real symmetry-breaking event: phi starts near the unstable midpoint and settles into a binary 90/10 split. The positive-phase occupancy grows from 0.8960 at t=0 to 0.8960 at t=999.
...
- After the transient, the system is not broadband-critical. For ticks >= 200, n_active locks into a narrow period-2.50 oscillation with lag-5 autocorrelation 0.733 and CV 0.0610.
...
- Interface energy localization is sharp: final cross-phase bond psi mean is 0.976, versus 0.690 inside domains.
...
- Bond maturation continues after the slab geometry freezes: bonds_total stabilizes near tick 108, while n_condensed_bonds does not settle within 1 percent of final until tick 799.
...
- The late-time active fraction is only 0.0923 of nodes on average, so the activity is sparse and spatially localized rather than globally distributed.
- There is persistent oscillation, but not the kind of broadband or avalanche-rich variability usually used as a brain-like criticality proxy. The late-time walker CV is 0.2191 and the dominant activity frequency is 0.40000 cycles per tick.
- The topology also stays simple: the largest condensed-bond component contains 0.806 of nodes, and there are no persistent new long-range edges in the snapshots.
