...
## Dataset
- H5 path: /home/justin/git/Prometheus_VDM/vdm_rt_v9/runs/20260414_050300_N512_pulse/run_log.h5
- Nodes: 512
- Lattice: 8 x 8 x 8 cubic
- Stimulus: pulse (amp=0.0)
- Summary ticks: 0 to 9999 (10000 samples)
- Snapshot cadence: every 50 ticks (201 snapshots)

## Executive Findings
- The run shows a real symmetry-breaking event: phi starts near the unstable midpoint and settles into a binary 90/10 split. The positive-phase occupancy grows from 0.8438 at t=0 to 0.8438 at t=9999.
- The settled geometry is a z-oriented slab, not a fragmented foam. The final positive planes are contiguous with thickness 8 out of 8 and x/y occupancy is flat to machine precision.
...
- The early transient is violent but short: walkers peak at 1047 on tick 64, kT peaks at 0.00285785 on tick 2, and active nodes peak at 355 on tick 2.
- After the transient, the system is not broadband-critical. For ticks >= 200, n_active locks into a narrow period-5.00 oscillation with lag-5 autocorrelation 0.968 and CV 0.1160.
...
## Stimulus Geometry
- The pulse logic injects 10 nodes per sign.
- Positive region bounds: [0, 0, 0] to [7, 1, 0]; z slices [0].
- Negative region bounds: [0, 5, 2] to [7, 6, 2]; z slices [2].
...
- Strong evidence for phase separation: final low/high well occupancy is 0.156/0.844, with middle occupancy 0.000 and bimodality coefficient 0.998.
- Macrostate lock occurs by snapshot tick 0, and full observation of the lattice is present by snapshot tick None.
- Interface energy localization is sharp: final cross-phase bond psi mean is 0.982, versus 0.769 inside domains.
- The final undirected interface count is 632, while the positive domain volume increases fivefold relative to the first snapshot. That is compatible with low-perimeter coarsening rather than branching hierarchy growth inside this seeded geometry.
- Bond maturation continues after the slab geometry freezes: bonds_total stabilizes near tick 87, while n_condensed_bonds does not settle within 1 percent of final until tick 1311.
...
- The late-time active fraction is only 0.1254 of nodes on average, so the activity is sparse and spatially localized rather than globally distributed.
- There is persistent oscillation, but not the kind of broadband or avalanche-rich variability usually used as a brain-like criticality proxy. The late-time walker CV is 0.1397 and the dominant activity frequency is 0.20000 cycles per tick.
- The topology also stays simple: the largest condensed-bond component contains 0.930 of nodes, and there are no persistent new long-range edges in the snapshots.
