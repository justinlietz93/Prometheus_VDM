...
# ─────────────────────────────────────────────────────────
#  GRAPH TOPOLOGY  (H5 snapshots → NetworkX → PyVis / GEXF)
# ─────────────────────────────────────────────────────────

def load_h5_snapshot(raw_bytes: bytes) -> Optional["nx.Graph"]:
    """
    Load a state_*.h5 snapshot and return a NetworkX graph.

    Tries common storage conventions used in VDM snapshots:
      - CSR: row_ptr / col_idx / data (weight)
      - Dense adj matrix: 'adj' or 'W'
      - Edge list: 'edges' (N×2) + optional 'weights'
    """
    if not HAS_H5PY or not HAS_PYVIS:
        return None
    try:
        with h5py.File(io.BytesIO(raw_bytes), "r") as f:
            ...
            # CSR format
            if "row_ptr" in keys and "col_idx" in keys:
                row_ptr = f["row_ptr"][:]
                col_idx = f["col_idx"][:]
                data    = f["data"][:] if "data" in keys else np.ones(len(col_idx))
                import scipy.sparse as sp
                N = len(row_ptr) - 1
                adj = sp.csr_matrix((data, col_idx, row_ptr), shape=(N, N))
                G = nx.from_scipy_sparse_array(adj, create_using=nx.Graph())

            # Dense adj / weight matrix
            elif "adj" in keys or "W" in keys:
                mat_key = "adj" if "adj" in keys else "W"
                mat = f[mat_key][:]
                if mat.ndim == 2:
            ...
            # Edge list
                ...
                edges = f["edges"][:]
                weights = f["weights"][:] if "weights" in keys else np.ones(len(edges))
                G = nx.Graph()
        ...
        # Remove self-loops; keep largest connected component
...
def graph_summary_stats(G: "nx.Graph") -> dict:
    """Compute degree / weight summary stats from a NetworkX graph."""
    if G is None or len(G) == 0:
        return {}
    degrees = np.array([d for _, d in G.degree()], dtype=float)
    weights = np.array([d["weight"] for _, _, d in G.edges(data=True)
                        if "weight" in d], dtype=float)
    return {
        "N_nodes": len(G),
        ...
        "gini_degree": gini_coeff(degrees),
        "mean_weight": float(np.mean(weights)) if weights.size else float("nan"),
        "density": nx.density(G),
    }


def render_pyvis(G: "nx.Graph", max_nodes: int = 800,
                 weight_col: str = "weight") -> str:
    """
    Render graph with PyVis (vis.js WebGL).  Returns full HTML string.
    Subsamples to max_nodes by degree (keeps hubs).
    """
    if not HAS_PYVIS or G is None:
        return "<p style='color:#64748b'>PyVis unavailable or no graph loaded.</p>"

    # Subsample: keep top-degree nodes
    if len(G) > max_nodes:
    ...
    net = PyvisNetwork(
        ...
        bgcolor=PANEL, font_color=FONT_COL,
    ...
    for node in G.nodes():
        ...
        size = 5 + 18 * (deg / max(max_deg, 1)) ** 0.5
        # color by degree: low=blue, high=orange
        t = deg / max(max_deg, 1)
        r = int(59 + (249 - 59) * t)
        g_ = int(130 + (115 - 130) * t)
        b = int(246 + (22 - 246) * t)
        color = f"#{r:02x}{g_:02x}{b:02x}"
        net.add_node(int(node), size=size, color=color,
                     title=f"node {node}  deg={deg}")

    for u, v, data in G.edges(data=True):
        w = float(data.get(weight_col, 1.0))
        net.add_edge(int(u), int(v), value=w, title=f"w={w:.3f}")

    html_str = net.generate_html()
    return html_str


def build_gexf_bytes(G: "nx.Graph") -> bytes:
    """Export graph as GEXF (Gephi native format)."""
    buf = io.BytesIO()
    nx.write_gexf(G, buf)
    return buf.getvalue()


def fig_graph_degree_dist(G: "nx.Graph") -> go.Figure:
    if G is None:
        return sfig("Degree Distribution (no graph)", height=360)
    degrees = np.array([d for _, d in G.degree()], dtype=float)
    d_sorted = np.sort(degrees)
    ccdf = 1 - np.arange(1, len(d_sorted) + 1) / len(d_sorted)
    xmin = max(1.0, float(np.quantile(d_sorted, 0.4)))
    alpha, ks, n_tail = powerlaw_mle(d_sorted, xmin)

    fig = sfig(f"Degree CCDF  N={len(G)}  α≈{alpha:.2f}", height=360)
    fig.add_trace(go.Scatter(x=d_sorted, y=ccdf, mode="markers",
                             marker=dict(size=3, color=ACCENT, opacity=0.7),
                             name="empirical"))
    if math.isfinite(alpha) and d_sorted.max() > xmin:
        xfit = np.logspace(np.log10(xmin), np.log10(d_sorted.max()), 40)
        fig.add_trace(go.Scatter(x=xfit, y=(xfit / xmin) ** (1 - alpha),
                                 mode="lines",
                                 line=dict(color="#ef4444", dash="dash", width=1.5),
                                 name=f"PL  α={alpha:.2f}  KS={ks:.3f}"))
    fig.update_xaxes(type="log", title_text="Degree")
    fig.update_yaxes(type="log", title_text="CCDF")
    return fig


def fig_graph_weight_dist(G: "nx.Graph") -> go.Figure:
    if G is None:
        return sfig("Edge Weight Distribution (no graph)", height=360)
    weights = np.array([d.get("weight", 1.0) for _, _, d in G.edges(data=True)], dtype=float)
    fig = sfig(f"Edge Weight Distribution  mean={np.mean(weights):.4f}", height=360)
    fig.add_trace(go.Histogram(x=weights, nbinsx=60,
                               marker_color=PALETTE[3], opacity=0.8))
    fig.update_xaxes(title_text="Weight")
    fig.update_yaxes(title_text="Count")
    return fig


def fig_laplacian_spectrum(G: "nx.Graph", k: int = 20) -> go.Figure:
    """Normalized Laplacian spectrum via sparse eigsh."""
    if G is None:
        return sfig("Laplacian Spectrum (no graph)", height=360)
    try:
        import scipy.sparse as sp
        ...
        vals, _ = spla.eigsh(L, k=kk, which="SM")
        vals = np.sort(np.real(vals))
    except Exception as e:
        return sfig(f"Laplacian Spectrum — error: {e}", height=360)

    fig = sfig(f"Normalized Laplacian Spectrum  (first {len(vals)} eigenvalues)", height=360)
    fig.add_trace(go.Bar(x=np.arange(len(vals)), y=vals,
                         marker_color=[PALETTE[0] if v < 0.05 else PALETTE[2]
                                       for v in vals], name="λ"))
    # Fiedler value annotation
    fiedler = vals[1] if len(vals) > 1 else float("nan")
    fig.add_annotation(x=1, y=fiedler,
                       text=f"λ₂ (Fiedler)={fiedler:.4f}",
...
NAV_ITEMS = [
    ...
    ("mip",             "∩  MIP / Integration"),
    ("topology",        "⬡  Topology  (H5)"),
    ("network",         "⬡  Network  (scalar)"),
    ("pci",             "Φ  PCI-like"),
...
app.layout = html.Div([
    ...
    dcc.Store(id="store-graph-html"),   # PyVis HTML string
    dcc.Store(id="store-graph-stats"),  # summary stats dict
    dcc.Store(id="store-gexf-b64"),     # base64 GEXF for download
    nav_sidebar(),
    html.Div([
        ...
        upload_bar(),
        # H5 snapshot upload row
        html.Div([
            dcc.Upload(
                id="upload-h5",
                children=html.Div([
                    html.Span("▲ Drop state_*.h5 snapshot  (optional — enables Topology section)",
                              style=dict(color=MUTED, fontSize="11px")),
                ]),
                style=dict(
                    padding="9px 18px", border=f"1px dashed {BORDER}",
                    borderRadius="6px", cursor="pointer",
                    background=BG, color=FONT_COL,
                    fontFamily="'JetBrains Mono', monospace",
                    display="inline-block", minWidth="420px",
                ),
                multiple=False,
            ),
            html.Span(id="h5-upload-status",
                      style=dict(marginLeft="14px", fontSize="11px",
                                 color=MUTED, fontFamily="'JetBrains Mono', monospace")),
            html.A(
                "⬇ Download GEXF (Gephi)",
                id="gexf-download-link",
                download="connectome.gexf",
                href="",
                target="_blank",
                style=dict(
                    marginLeft="20px", fontSize="11px",
...
@app.callback(
    Output("store-graph-html",  "data"),
    Output("store-graph-stats", "data"),
    Output("store-gexf-b64",    "data"),
    Output("h5-upload-status",  "children"),
    Output("gexf-download-link","href"),
    Output("gexf-download-link","style"),
    Input("upload-h5", "contents"),
    State("upload-h5", "filename"),
    prevent_initial_call=True,
)
def process_h5_upload(contents, filename):
    _hidden = dict(marginLeft="20px", fontSize="11px", color=ACCENT,
                   fontFamily="'JetBrains Mono', monospace",
                   textDecoration="none", display="none")
    _visible = {**_hidden, "display": "inline"}

    if contents is None:
        return no_update, no_update, no_update, "", "", _hidden

    if not HAS_H5PY or not HAS_PYVIS:
        return (no_update, no_update, no_update,
                "✗ h5py / pyvis not installed", "", _hidden)
    try:
        _, b64 = contents.split(",", 1)
        raw = base64.b64decode(b64)
        G = load_h5_snapshot(raw)

        if G is None:
            return (no_update, no_update, no_update,
                    "✗ Could not parse graph from H5 (check key names: row_ptr/col_idx, adj, W, or edges)",
                    "", _hidden)

        stats = graph_summary_stats(G)
        pyvis_html = render_pyvis(G, max_nodes=800)
        gexf_bytes = build_gexf_bytes(G)
        gexf_b64   = base64.b64encode(gexf_bytes).decode()
        gexf_href  = f"data:application/gexf+xml;base64,{gexf_b64}"

        status = (f"✓ {filename}  |  N={stats['N_nodes']:,}  "
                  f"E={stats['N_edges']:,}  ⟨k⟩={stats['mean_degree']:.1f}  "
                  f"Gini={stats['gini_degree']:.3f}")

        return pyvis_html, json.dumps(stats), gexf_b64, status, gexf_href, _visible
...
@app.callback(
    Output("topology-pyvis-frame", "srcDoc"),
    Input("store-graph-html", "data"),
    prevent_initial_call=True,
)
def update_pyvis_frame(html_str):
    if not html_str:
        return "<p style='color:#64748b;font-family:monospace;padding:20px'>No graph loaded.</p>"
    return html_str
...
def render_dashboard(tick_json, status_json):
    ...
    # -- TOPOLOGY (H5 graph) --
    topo_sec = section("topology", "TOPOLOGY  (H5 snapshot — PyVis · Gephi · Laplacian)", [
        html.Div([
            html.Div(
                "Drop a state_*.h5 snapshot in the upload bar above to activate this section. "
                "The graph renders interactively below via PyVis (vis.js). "
                "Use the ⬇ Download GEXF button to open in Gephi for publication-quality figures.",
                style=dict(color=MUTED, fontSize="11px",
                           fontFamily="'JetBrains Mono', monospace",
                           marginBottom="14px"),
            ),
            # PyVis iframe — srcDoc populated by callback
            html.Iframe(
                id="topology-pyvis-frame",
                srcDoc="<p style='color:#64748b;font-family:monospace;padding:20px'>No graph loaded — upload a state_*.h5 file.</p>",
                style=dict(
        ...
        html.Div([
            html.Div("Degree distribution and Laplacian spectrum load automatically after H5 upload.",
                     id="topo-stats-hint",
                     style=dict(color=MUTED, fontSize="11px",
                                fontFamily="'JetBrains Mono', monospace",
                                marginTop="14px")),
        ]),
        # Degree + Laplacian figures — rendered via separate callback
        html.Div(id="topo-figures-row"),
    ])

    # -- NETWORK (scalar proxy) --
    ...
    net_sec = section("network", "NETWORK  (scalar proxy)", [
    ...
    return html.Div([
        ...
        mip_sec, topo_sec, net_sec, pci_sec, pmi_sec, ro_sec,
        sp_sec, tco_sec, bio_sec, sf_sec, rg_sec,
    ])


@app.callback(
    Output("topo-figures-row", "children"),
    Output("topo-stats-hint",  "children"),
    Input("store-graph-stats", "data"),
    Input("store-graph-html",  "data"),
    State("upload-h5", "contents"),
    State("upload-h5", "filename"),
    prevent_initial_call=True,
)
def update_topo_figures(stats_json, graph_html, h5_contents, h5_filename):
    """Re-parse the H5 to build Plotly figures for degree dist + Laplacian."""
    if not stats_json or not h5_contents or not HAS_H5PY or not HAS_PYVIS:
        return no_update, no_update

    try:
        _, b64 = h5_contents.split(",", 1)
        raw = base64.b64decode(b64)
        G = load_h5_snapshot(raw)
        if G is None:
            return no_update, no_update
        ...
        deg_fig = fig_graph_degree_dist(G)
        wt_fig  = fig_graph_weight_dist(G)
        lap_fig = fig_laplacian_spectrum(G, k=20)

        stats_text = (
            f"N={stats['N_nodes']:,}  ·  E={stats['N_edges']:,}  ·  "
            f"⟨k⟩={stats['mean_degree']:.2f}  ·  k_max={int(stats['max_degree'])}  ·  "
            f"Gini(k)={stats['gini_degree']:.3f}  ·  density={stats['density']:.5f}"
        )

        row = html.Div([
            html.Div(
                dcc.Graph(figure=deg_fig, config=dict(displayModeBar=False)),
                style=dict(flex="1", minWidth="0"),
            ),
            html.Div(
                dcc.Graph(figure=wt_fig, config=dict(displayModeBar=False)),
                style=dict(flex="1", minWidth="0"),
            ),
            html.Div(
                dcc.Graph(figure=lap_fig, config=dict(displayModeBar=False)),
                style=dict(flex="1", minWidth="0"),
            ),
        ], style=dict(display="flex", gap="12px", flexWrap="wrap", marginTop="14px"))

        return row, stats_text

    except Exception as e:
        return no_update, f"Error building figures: {e}"
