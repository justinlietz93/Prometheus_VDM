...
* **General Simulation Parameters:**
  ...
  * **Higgs Reference Scale (legacy FUM proof, non-canonical):** In the speculative FUM-era void-dynamics proofs in [`VDM-Initial-Proofs.md`](Derivation/Speculations/VDM-Initial-Proofs.md:1), a `higgs_reference_scale` was computed by dividing the experimental Higgs mass (124.0 GeV) by a toy `final_vev`, and `predicted_higgs_mass` obtained by multiplying `final_vev` by this scale. This construction is preserved only as a historical calibration example and **must not** be treated as a predictive or required procedure for VDM experiments.
  * **Light Speed Scale (legacy FUM proof, non-canonical):** In the same speculative note, a `light_speed_scale` was defined by dividing the fixed `target_light_speed` (299,792,458 m/s) by a toy `final_value`, with `predicted_light_speed = final_value * light_speed_scale`. This should be read purely as a units-calibration toy; the physical speed of light `c` is fixed by definition and by canonical KG-cone gates, and **no VDM experiment may claim to predict `c` from this construction.**
