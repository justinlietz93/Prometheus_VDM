...
- Instrument certification scaffolding and policy are implemented in `Derivation/code/common/certified_instruments/`, domain `APPROVAL*.json`, and proposal/prereg artifacts, with enforcement wired through approvals and policy tests.
...
- Governed runners import shared helper modules for meters, validation gates, plotting, and IO routing; this separation is actively tested via policy enforcement checks.
...
- Runners operate from prereg/spec JSON payloads as the numerical source of truth, with CLI primarily used to select the spec artifact.
- Domain tests exercise spec-driven runs and schema-required outputs, reinforcing prereg/spec-driven parameter authority.


## 2.8 Evidence snapshot (implemented controls in code)

Representative code-level evidence confirming the above implementation status:

- Policy test enforcing approval/io helper usage in runnable scripts: `Derivation/code/tests/test_policy_enforcement.py`.
- Validation-gate helper separation contract (meters vs gates vs io routing): `Derivation/code/common/validation_gate_helpers/README.md`.
- SQLite-backed approvals (`approvals`, `domain_keys`, `tag_secrets`, `exempt_scripts`) and `VDM_APPROVAL_DB` resolution: `Derivation/code/common/authorization/approval.py`.
- Domain runner importing helper stack (approval, io_paths, plotting, instrument helpers, validation helpers): `Derivation/code/physics/metriplectic/T2_Metriplectic_Instruments_v1.py`.
- Spec-driven parameter ingestion and approval-gated artifact-writing path: `Derivation/code/physics/cosmology/void_lensing/experiments/T2_void_lensing_meter_synthetic_mocks_v1.py`.
- Domain tests validating spec/schema-driven artifacts and run receipts: `Derivation/code/tests/cosmology/void_lensing/test_void_lensing_meter_experiment.py`.
...
- Existing helper families are used in production runners (instrument helpers, validation-gate helpers, plotting, IO path routing).
- Policy tests are present to enforce approval + IO helper import discipline for runnable physics scripts.

**Remaining hardening opportunities**
- Add always-on architectural boundary checks for presentation/application/domain/infrastructure import edges.
- Promote current JSON artifact conventions (ProposalDocument, ApprovalArtifact, PredictionEnvelope, etc.) into explicit typed contract packages for stronger static guarantees.
...
- Parameters and test inputs are driven by prereg/spec artifacts in governed runners.

**Remaining hardening opportunities**
- Standardize contradiction report schema + publication index contract across every domain.
- Make prediction-envelope artifacts explicit and mandatory in every domain pipeline.
