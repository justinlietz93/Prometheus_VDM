...
```math
mod\_factor = 2 \sigma(R_{\mathrm{total}}) - 1 , where R_{\mathrm{total}}
```
 ...
 ```math
 \Delta w_{ij} \propto \eta \cdot (1 + mod\_factor) \cdot R_{\mathrm{total}} \cdot e_{ij} 
 ```
...
```math
\Delta W = \left[ \eta \cdot (1 + mod\_factor) \cdot R_{\mathrm{total}} \cdot E \right] \cdot \Delta t - \lambda W \cdot \Delta t
```
   ...
   * Where:
       ...
       * $mod_factor = 2 \sigma(R_\{\mathrm{total}}) - 1$: The non-linear reward modulation factor.
       * $E$: Matrix of eligibility traces ($e\_{ij}$).
...
3.  **Lyapunov Stability Analysis (Preliminary & Refined):**
...
* **Change Analysis:** The change $\Delta L$ per update step is approximately $\Delta L \approx \langle W, \Delta W \rangle_F$. Substituting the $\Delta W$ equation (assuming $\Delta t=1$ or absorbed, and defining the effective learning rate $\eta_{\mathrm{eff}} = \eta \cdot (1 + mod\_factor)$) yields:
...
4.  **Convergence Analysis (Planned):** Identifying fixed points ($\Delta W = 0$, $\Delta V = 0$) of the refined system and analyzing their stability. Deriving bounds on reward variance.

5.  **Multi-Objective Analysis (Planned):** Using Pareto optimality concepts to analyze trade-offs between SIE components.

6.  **Gaming Analysis (Planned):** Identifying parameter regimes prone to reward hacking.
...
- **Modulation Factor ($mod_factor$):** Appears stable (Mean=0.0148, StdDev=0.0417).
