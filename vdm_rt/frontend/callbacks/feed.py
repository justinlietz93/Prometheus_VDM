...
from vdm_rt.frontend.utilities.pdf_utils import convert_pdf_to_text_file
...
def register_feed_callbacks(app, manager, repo_root: str):
    ...
    Feed controls:
      ...
      - Stop feeding
    Uses the same IDs as the inline version in vdm_live to preserve behavior.
    """
    ...
    def on_feed_start(_n, path, rate):
        p = (path or "").strip()
        if not p:
            return "Provide a feed path (relative to vdm_rt/data or absolute)."
        chosen = p
        try:
