...
This is a **minimal, production‑oriented** runtime that matches the Nexus ⇄ UTE/UTD vision.
It runs continuously, ingests input, updates the connectome with the *void equations* if
present (`Void_Equations.py`), logs metrics, and renders dashboards and connectome images.

> Entry point: `python -m vdm_rt.run_nexus`
...
export PYTHONPATH=.
python -m vdm_rt.run_nexus --neurons 800 --hz 10 --domain biology_consciousness --viz-every 5
```
...
If the repo already contains `Void_Equations.py` and `Void_Debt_Modulation.py` on `PYTHONPATH`,
this runtime will import them automatically.

If not, drop those files at the project root (next to `vdm_rt/`) **or** copy them into `vdm_rt/core/`.
The adapter will prefer the versions and only fall back to an internal stub if not found.
...
2. Connectome applies the void dynamics to the node field vector `W` (vectorized).
...
```bash
python -m vdm_rt.run_nexus [--neurons N] [--k K] [--hz HZ] [--domain NAME]
...
- `vdm_rt/nexus.py` - the real‑time orchestrator.
- `vdm_rt/core/void_dynamics_adapter.py` - loads the void functions or a minimal stub.
- `vdm_rt/core/connectome.py` - kNN‑ish graph + vectorized update step.
- `vdm_rt/core/metrics.py` - sparsity/cohesion/complexity metrics.
- `vdm_rt/core/visualizer.py` - dashboard & graph rendering (matplotlib).
- `vdm_rt/core/memory.py` - engram snapshots (.npz).
- `vdm_rt/io/ute.py` - Universal Temporal Encoder (stdin & synthetic tick sources).
- `vdm_rt/io/utd.py` - Universal Transduction Decoder (stdout & file sink).
- `vdm_rt/utils/logging_setup.py` - structured logger helper.
...
- On startup, the Nexus also registers macro keys from:
  ...
  2) `vdm_rt/io/lexicon/macro_board_min.json` (fallback).
...
- Macro board (auto‑persisted each run): runs/&lt;timestamp&gt;/macro_board.json
  - Runtime source: [vdm_rt/io/utd.py](vdm_rt/io/utd.py)
  - Nexus reads macros at boot: [vdm_rt/nexus.py](vdm_rt/nexus.py)
- Phrase bank (optional source of sentence templates, loaded at boot):
  - Per‑run: runs/&lt;timestamp&gt;/phrase_bank.json
  - Fallback: [vdm_rt/io/lexicon/phrase_bank_min.json](vdm_rt/io/lexicon/phrase_bank_min.json)
...
- The runtime loads optional sentence templates for the “say” macro from either:
  ...
  - [vdm_rt/io/lexicon/phrase_bank_min.json](vdm_rt/io/lexicon/phrase_bank_min.json)
- Expected shape:
...
- The runtime maintains runs/&lt;timestamp&gt;/lexicon.json as a simple token frequency store learned from:
  - Inbound UTE text messages (the input stream)
  - Emitted “say” lines
...
- The more diverse and structured the input, the richer the learned vocabulary.
...
- Purpose: Selects a modulation factor for the void equations, scaling both the growth and decay elemental deltas before each tick. The value is computed by [get_domain_modulation()](vdm_rt/core/void_dynamics_adapter.py:46), and is passed into both Δα and Δω inside the adapter.
- How it works:
  - If you provide the own Void_Debt_Modulation on PYTHONPATH (class with get_universal_domain_modulation), the adapter uses it to obtain domain_modulation.
...
- Supported presets (fallback path) include: quantum, standard_model, dark_matter, biology_consciousness, cosmogenesis, higgs. Any unknown string (e.g., "math_physics") resolves to a baseline default in the fallback path unless the module overrides it.
...
- Where: See default profile definitions in [Nexus._default_phase_profiles()](vdm_rt/nexus.py:339).
- How it’s applied:
  - The runtime polls runs/{timestamp}/phase.json each tick via [Nexus._poll_control()](vdm_rt/nexus.py:403).
  - When the file exists and its mtime changes, the profile is merged and applied immediately. The path is set at startup in [Nexus.**init** → phase_file](vdm_rt/nexus.py:183).
- On vs Off:
...
- Implementations:
  - Dense backend: [Connectome.cyclomatic_complexity()](vdm_rt/core/connectome.py:375)
  - Sparse backend: [SparseConnectome.cyclomatic_complexity()](vdm_rt/core/sparse_connectome.py:393)
- How it’s used:
  - The Nexus uses complexity_cycles each tick as a “B1 proxy” input to a streaming z‑score detector. See the B1 update inside [Nexus.run()](vdm_rt/nexus.py:442) where b1_value is taken from m["complexity_cycles"] and fed to the z‑spike detector to gate speaking.
...
- Domain: CLI/profile string selecting void‑equation modulation; fallback mapping is internal unless you provide the own implemention. See [get_domain_modulation()](vdm_rt/core/void_dynamics_adapter.py:46).
- Phase control: Enabled when runs/{timestamp}/phase.json exists; disabled when it doesn’t. Profiles are defined in [Nexus._default_phase_profiles()](vdm_rt/nexus.py:339), polled by [Nexus._poll_control()](vdm_rt/nexus.py:403), and applied live.
