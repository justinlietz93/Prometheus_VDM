...
"""
File Picker Common Callback Registrar

Purpose:
- Provide a modular registrar for all common file picker callbacks
- Keep IO bounded (single-directory listings), prevent scans in core/ or maps/
- Centralize status computation, breadcrumbs, and tree rendering
- Decouple from specific roots by accepting project_root param

This module is imported by specialized registrars (static/dynamic roots).
"""
...
def register_file_picker_common(app, prefix: str, target_id: str, project_root: str) -> None:
    ...
    for a file-picker instance. The 'open' callbacks are registered by the caller.
