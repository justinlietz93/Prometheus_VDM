...
"""
File Picker Registrars (thin wrappers)

- Static root registrar
- Dynamic (engram) root registrar

Both delegate all common navigation/tree/file/confirm behavior to:
  fum_rt.frontend.callbacks.file_picker_common.register_file_picker_common

Design:
- Bounded IO (single-directory listings only)
- No scans in core/ or maps/ (enforced in controller)
- Centralized status computation (directory vs file) via controllers
- Reusable widgets (tree rows, breadcrumbs)
"""
...
from fum_rt.frontend.callbacks.file_picker_common import register_file_picker_common
...
def _project_root_default() -> str:
    ...
    """
    try:
        env = os.path.abspath(os.getenv("FUM_PROJECT_ROOT", "").strip())
        if env and os.path.isdir(env):
            return env
    except Exception:
        pass
    # callbacks/ -> frontend/ -> .. (repo root)
