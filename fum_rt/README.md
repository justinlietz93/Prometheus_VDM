...
- `fum_rt/core/void_dynamics_adapter.py` - loads the void functions or a minimal stub.
...
- How it’s applied:
  ...
  - When the file exists and its mtime changes, the profile is merged and applied immediately. The path is set at startup in [Nexus.**init** → phase_file](fum_rt/nexus.py:183).
- On vs Off:
...
- Domain: CLI/profile string selecting void‑equation modulation; fallback mapping is internal unless you provide the own implemention. See [get_domain_modulation()](fum_rt/core/void_dynamics_adapter.py:46).
