# CF02: Complete Formalism — Contact Geometry to Metriplectic Evolution
...
- **Classification:** Axiom-core (geometric origin of A4) + derived-limit checks.
- **Domain of validity:** Smooth contact manifold `(M^{2n+1}, α)` with well-defined Reeb field and contact Hamiltonian `h`. We treat Legendre submanifolds explicitly when passing to equilibrium thermodynamics.

### Primary claims

- **C1 (Contact → evolution):** Contact Hamiltonian dynamics generated by `(α, h)` defines a well-posed flow on `M`.
- **C2 (Split into reversible/irreversible):** The contact flow decomposes into a symplectic/Hamiltonian part (reversible) plus a metric/gradient part (irreversible), matching the metriplectic `J ⊕ M` split once restricted to the physical state manifold.
- **C3 (A4 compatibility):** Under the stated identification of `(I, Σ)`, the induced operators satisfy `J^T = -J`, `M^T = M ≥ 0`, plus the degeneracy constraints `J·δΣ = 0` and `M·δI = 0`.
- **C4 (Entropy law):** The resulting irreversible component obeys an H-theorem: `dΣ/dt ≥ 0` with equality only at steady states.

### Assumption ledger

- **A1:** `(M, α)` is a contact manifold: `α ∧ (dα)^n ≠ 0` everywhere on the domain.
- **A2:** The contact Hamiltonian `h` is smooth and yields complete flow for the considered initial conditions (or else we restrict to maximal interval).
- **A3:** When we project/restrict from contact space to the VDM state space, the projection map is smooth and preserves the required brackets (explicitly specified; no hidden identifications).

### Decisive falsifiers / gates

- **G1 (Contact identities):** Reeb field computed from `ι_R α = 1` and `ι_R dα = 0` satisfies both conditions to tolerance.
- **G2 (Bracket symmetries):** Constructed `J` is antisymmetric and constructed `M` is symmetric PSD (numeric eigen-check).
- **G3 (Degeneracy):** `J·δΣ` and `M·δI` residuals are ≤ ε on representative states.
- **G4 (Entropy production):** Measured `ΔΣ` over time steps is ≥ −ε_Σ (monotonic within tolerance).
- **G5 (Coordinate invariance):** Results are invariant under contactomorphisms / coordinate changes tested in CFN.

### CFN outputs

- explicit forms of `α`, `dα`, `R`, and the contact bracket components;
- the derived `J` and `M` tensors/operators and their symmetry checks;
- time-series of `I` and `Σ` verifying degeneracy and H-theorem;
- provenance (seed, discretization, solver settings, commit hash).
