...
\author{
  ...
  \href{https://orcid.org/0009-0008-9028-1366}{\includegraphics[height=1.5ex]{orcid.pdf}~0009-0008-9028-1366}
}
...
\texttt{void\_lensing\_meter-mocks-grid-v3.json} that spans three mocks
families --- \texttt{PyTwinPeaks}, \texttt{FlagshipLike}, and
\texttt{stress\_test} --- over two tomographic bins
$z\in[0.2,0.6]$ and $z\in[0.6,1.0]$ and a void-radius range
$R_v\in[10,30]~\mathrm{Mpc}$ (comoving), while retaining the
same meter core and gate thresholds and reusing the control parameters
($n_{\text{radial bins}} = 30$,
$x_{\text{wall range}} = [0.8, 1.2]$,
$x_{\text{bg range}} = [2.5, 4.0]$,
$\lambda_{\text{ref}} = 1.0$,
...
\texttt{20251201\_040626\_T2\_void\_lensing\_meter\_synthetic\_mocks\_v1}\\
\texttt{\_void\_lensing\_meter-mocks\_void\_lensing\_meter-v1}
and live here as:
\begin{itemize}
  \item \path{logs/20251201_040626_T2_void_lensing_meter_synthetic_mocks_v1_void_lensing_meter-mocks_void_lensing_meter-v1_runs.json}
  \item \path{logs/20251201_040626_T2_void_lensing_meter_synthetic_mocks_v1_void_lensing_meter-mocks_void_lensing_meter-v1_gates.json}
  \item \path{logs/20251201_040626_T2_void_lensing_meter_synthetic_mocks_v1_void_lensing_meter-mocks_void_lensing_meter-v1_runs.csv}
  \item \path{figures/20251201_040626_T2_void_lensing_meter_synthetic_mocks_v1_void_lensing_meter-mocks_void_lensing_meter-v1_profile.png}
\end{itemize}
...
\texttt{20251201\_153008\_T2\_void\_lensing\_meter\_synthetic\_mocks\_v1}\\
\texttt{\_void\_lensing\_meter-mocks-grid\_void\_lensing\_meter-v1}
with artifacts:
\begin{itemize}
  \item \path{logs/20251201_153008_T2_void_lensing_meter_synthetic_mocks_v1_void_lensing_meter-mocks-grid_void_lensing_meter-v1_runs.json}
  \item \path{logs/20251201_153008_T2_void_lensing_meter_synthetic_mocks_v1_void_lensing_meter-mocks-grid_void_lensing_meter-v1_gates.json}
  \item \path{logs/20251201_153008_T2_void_lensing_meter_synthetic_mocks_v1_void_lensing_meter-mocks-grid_void_lensing_meter-v1_runs.csv}
  \item \path{figures/20251201_153008_T2_void_lensing_meter_synthetic_mocks_v1_void_lensing_meter-mocks-grid_void_lensing_meter-v1_profile.png}
\end{itemize}
and gate receipt:
\begin{itemize}
...
H1--H3 on the tuned v3 synthetic-mocks grid, without any change to the
meter core or gate thresholds, and thus completes the T2
instrument-calibration objectives stated in the proposal.
...
Therefore, on the tuned v3 synthetic-mocks grid described above (with
the canonical PyTwinPeaks calibration as a limiting case), the
...
with negligible bias. This statement is strictly limited to the
synthetic-mocks domain; no real-data performance or cosmological
inference is claimed here.

\subsection*{Instrument status and domain}
As of 2025-12-01 we regard \texttt{void\_lensing\_meter-v1} as
T2-certified only on the tuned v3 synthetic-mocks grid with
\texttt{backend}~$\in\{\texttt{PyTwinPeaks},\texttt{FlagshipLike},\texttt{stress\_test}\}$,
$z\in[0.2,0.6]\cup[0.6,1.0]$, and
$R_v\in[10,30]~\mathrm{Mpc}$ (comoving), under the control parameters
documented in the Methods section. The meter has not yet been certified
on any real convergence maps (e.g., HSC S16, ACT, DES) or survey-specific
systematics, and no T3+ cosmological inferences are drawn in this paper.
Instead, the v1 instrument, calibrated on this v3 grid, is intended to
serve as the baseline against which any future
\texttt{void\_lensing\_meter-v2} extensions and real-data pipelines will
be measured.
...
\nocite{
  Lietz_VDM_Instruments_and_2025,
  shi2025akramassmapreconstruction,
  boruah2025highresolutionweaklensingmass,
  zhou2025mapbasedebseparationfiltered,
  Maggiore_2025,
  kanafi2025quantifyingweightedmorphologicalcontent,
  Su_2025,
  dejanvry2025calibrationtomographicredshiftdistribution,
  mccarthy2025atacamacosmologytelescopecrosscorrelation
}
