...
**Repository discipline** mirrors your template (approvals, prereg, schemas/specs). 
...
```json
{
  ...
  "pre_registered": true,
  "proposal": "Derivation/Gravity_Analog/T2_PROPOSAL_VDM_Ringdown_Meter_v1.md",
...
1. **Preflight (determinism & cones):** Run J‑only KG on two grids, confirm dispersion & locality receipts match prior gates (R²≥0.999, cone slope ≤1.02). This re‑anchors c, m. 
...
4. **Energy & flux:** Compute (\mathcal H) in (\Omega_R), boundary flux via (\mathbf S), and DG decrement; check budget:
   [
   \Delta E_{\Omega} + \int_0^t!!\int_{\partial\Omega}\mathbf S!\cdot!n,dA,dt' + \Delta \mathcal L_{\text{DG}} \approx 0.
   ]
...
**RQ:** *Does the VDM metriplectic scalar field produce a reproducible, meter‑grade ringdown observable—(ω, α, Q) and energy‑flux balance—that obeys finite‑speed locality and dimensionless scaling under variations of (R,c,m_{\rm eff},\varepsilon_M,\eta)?*
...
* Deterministic FP64; stepper tolerances logged; cross‑arch tolerance set (e.g., ≤1e‑12 for conserved quantities on J‑limb).
...
* **Figure 2.** **Flux balance plot:** cumulative (-\Delta E_{\Omega}) vs. (\int \mathbf S!\cdot!n) + DG decrement (slope ≈ 1).
...
If gates pass, VDM has a **certified ringdown meter**: (i) finite‑speed propagation (cone ≤1.02), (ii) monotone Lyapunov steps on M‑limb, (iii) reliable ((\omega,\alpha,Q)) extraction with ≤2–5% errors, and (iv) correct energy‑flux accounting using canon Poynting form. This creates the principled, first‑principles instrument needed **before** comparing to astrophysical ringdowns.
...
1. Create `Derivation/Gravity_Analog/` and drop this proposal as `T2_PROPOSAL_VDM_Ringdown_Meter_v1.md`.
...
4. Execute the R‑sweep spec (5 seeds), emit artifacts and PASS/FAIL JSON.
5. If all gates pass, post RESULTS with figure captions carrying numbers+R² and link this PROPOSAL in provenance.
