# Runtime Physics Migration Directive v4
...
def _propose_new_edges(self, phi_dot_abs: np.ndarray):
    ...
    for i in range(self.N):
        ...
        r = self.r_causal[i]
        if r < 1.5:
            continue  # cone hasn't reached beyond face neighbors
        max_shell = min(int(r), side // 2)
        if max_shell < 2:
            continue
        existing = set(self.adj[i].tolist())
        pi = self.pos[i]
        # Scan shells 2..max_shell
        for j in range(self.N):
            if j == i or j in existing:
                continue
            if phi_dot_abs[j] <= kT:
                continue
            dist = float(np.linalg.norm(pi - self.pos[j]))
...
### 5.2 Remove SIE drive computation

DELETE all code that computes `sie_drive`, `sie_gate`, `dom_mod`, `domain_modulation`, `idf_scale`, `density`, `td_signal`, `firing_var`. DELETE the entire block from "1) density from active edges" through "4) SIE drive" inclusive.
...
### 5.4 Metrics from physics reward

After the step call, replace the old metrics collection with:
...
# Physics observables from connectome
...
drive = {"valence_01": float(getattr(nx.connectome, "_last_sie2_valence", 0.0))}
