...
- `EQUATIONS.md#vdm-e-001` (Agency field evolution)
...
LOOP (per tick):
  ...
  4. Run optional REV-GSP learner (if ENABLE_REVGSP=1)        # fum_rt/runtime/loop/main.py:565
...
```text
INIT:
  - Compute d_alpha = delta_re_vgsp(W, t, domain_modulation, use_time_dynamics)
  - Compute d_omega = delta_gdsp(W, t, domain_modulation, use_time_dynamics)
  - a = ReLU(d_alpha) + external_stimulus (with decay)      # fum_rt/core/connectome.py:283
  - om = d_omega                                             # fum_rt/core/connectome.py:284

ALIAS TABLE BUILD:
  - Build alias sampler ~ a distribution                     # via _build_alias (Vose's method)
                                                             # fum_rt/core/connectome.py:96

STRUCTURAL PLASTICITY (per node):
  IF structural_mode == "dense" AND N <= 4096:              # validation/small N
    - Compute full affinity matrix S[i,j] = a[i]*a[j] - lambda_omega*|om[i]-om[j]|
    - Extract top-k neighbors per node via argpartition     # fum_rt/core/connectome.py:301
  ELSE:                                                      # default: alias sampling
    FOR each node i:
      - Sample s candidates ~ alias table (drop self/dupes) # fum_rt/core/connectome.py:311
      - Compute affinity w[j] = a[i]*a[j] - lambda_omega*|om[i]-om[j]|
      - Keep top-k by affinity as new neighbors             # fum_rt/core/connectome.py:314

SYMMETRIZE:
  - A_new = A_new OR A_new.T  (undirected graph)            # fum_rt/core/connectome.py:326

NODE FIELD UPDATE:
  - Apply universal_void_dynamics(W, t, domain_mod, use_time_dynamics) → delta_W
  - Gate by SIE: delta_W *= sie_drive                       # fum_rt/core/connectome.py:331
  - Update: W = clip(W + delta_W, 0, 1)                     # fum_rt/core/connectome.py:333

FINALIZE:
  - Store A_new → self.A                                    # fum_rt/core/connectome.py:335
  - Recompute edge weights E from W and A                   # fum_rt/core/connectome.py:337
```

**Preconditions:**
- N, k, threshold, lambda_omega, candidates configured
- W initialized (node weights [0,1])
- Alias sampler functions available (delta_re_vgsp, delta_gdsp, universal_void_dynamics)

**Postconditions/Invariants:**
- A (adjacency) is symmetric and has ~k neighbors per node (approximately)
- W stays bounded in [0,1]
- E (edge weights) derived from W and A

**Concurrency/Ordering:**
- Single-threaded; node updates are sequential in alias mode
- Dense mode uses vectorized NumPy (parallel on multi-core systems)

**Failure/Backoff hooks:**
- External stimulus accumulation wrapped in try-except (silent no-op)

**Emits/Side effects:**
- self.findings dict updated (vt_visits, vt_entropy, coverage)
- Bus: cycle_hit events (during void traversal), region_stat (end of traversal)
...
#### VDM-A-006 — REV-GSP Learning Step (Three-Factor Synaptic Plasticity)  <a id="vdm-a-006"></a>
**Context:** fum_rt/fum_advanced_math/void_dynamics/apply_revgsp.py:224-292 • Commit: 7498744 • Module: fum_advanced_math/void_dynamics

**Role:** Complete RE-VGSP learning step implementing three-factor rule: spike-timing (local) + eligibility (memory) + reward (global).

**Inputs:**
- synaptic_weights: sparse CSR matrix
- eligibility_traces: torch.Tensor (eligibility memory)
- spike_data: {"pre_spike_times": list, "post_spike_times": list}
- total_reward: float (SIE reward signal)
- plv: float (Phase-Locking Value for resonance)
- neuron_polarities: torch.Tensor ([-1, +1] per neuron)
- Constants: base_eta, base_gamma, lambda_decay

**Depends on equations:**
- TODO: add anchor for RE-VGSP formulas in EQUATIONS.md (calculate_modulated_learning_rate, update_eligibility_trace, calculate_weight_change)

**Pseudocode:**
```text
INIT:
  - Extract spike pairs using extract_spike_pairs_from_synapses (O(N) where N=nnz synapses)
                                                             # apply_revgsp.py:266
  IF no spike pairs:
    - eta_eff = calculate_modulated_learning_rate(base_eta, total_reward)
    - weight_changes = -lambda_decay * weights (decay only)
    - eligibility_traces *= base_gamma                      # apply_revgsp.py:286
    - RETURN {weight_changes, eligibility_traces, ...}

SPIKE PAIR PROCESSING:
  - Calculate pre/post phases for each spike pair          # apply_revgsp.py:296
  - Compute delta_t (post_time - pre_time)                 # apply_revgsp.py:77
  - Calculate plasticity impulses via calculate_plasticity_impulse(delta_t, plv, neuron_polarities)
                                                             # apply_revgsp.py:?? (imported formula)

ELIGIBILITY UPDATE:
  - gamma_eff = calculate_modulated_trace_decay(base_gamma, total_reward)
  - eligibility_traces = update_eligibility_trace(eligibility_traces, plasticity_impulses, gamma_eff)
                                                             # apply_revgsp.py:?? (imported formula)

WEIGHT UPDATE:
  - eta_eff = calculate_modulated_learning_rate(base_eta, total_reward)
  - weight_changes = calculate_weight_change(eligibility_traces, total_reward, eta_eff, lambda_decay, synaptic_weights)
                                                             # apply_revgsp.py:?? (imported formula)

RETURN:
  - {weight_changes, eligibility_traces, plasticity_impulses, eta_effective, gamma_effective, n_spike_pairs, complexity_validation}
                                                             # apply_revgsp.py:284
```

**Preconditions:**
- synaptic_weights is sparse CSR (existing synapses only)
- spike_data contains valid spike times within time_window
- neuron_polarities.size matches synaptic_weights.shape[0]

**Postconditions/Invariants:**
- Weight changes bounded by eta_eff and lambda_decay
- Eligibility traces decay over time (gamma_eff)

**Concurrency/Ordering:**
- O(N × S) where N=nnz synapses, S=avg spikes per neuron
- Sequential processing (no parallelism in current impl)

**Failure/Backoff hooks:**
- If no spike pairs: decay-only mode (silent fallback)     # apply_revgsp.py:273

**Emits/Side effects:**
- Logs: debug messages on spike pair count, SIE handshake  # apply_revgsp.py:280
...
- [VDM-A-006](#vdm-a-006) — REV-GSP Learning Step (Three-Factor Synaptic Plasticity)
